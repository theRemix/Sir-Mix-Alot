...
function Model(theSchema){
  ...
  //is this object already a data type in the datastore?
  if(!DataStore.store.hasOwnProperty[myCreator]) {
    DataStore.store[myCreator] = [];
  }
  console.log(DataStore.store[myCreator].length);

  Model.getNextId = function(){
    return DataStore.store[myCreator].length + 1;
  };
